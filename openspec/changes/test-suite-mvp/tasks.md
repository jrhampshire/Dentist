...
| 2.5 | `backend/tests/unit/test_pdf_service.py` | Create | [x] Test `generate_invoice_pdf`: ReportLab path (verify bytes start with `%PDF-`), ImportError fallback (verify HTML contains invoice data), both paths fail (raises `PDFGenerationError`), zero line items. Mock `import` to raise ImportError for fallback. No DB. ~100 lines. |
| 2.6 | `backend/tests/unit/test_exceptions.py` | Create | [x] Use DRF `APIRequestFactory` to create mock requests. Call `unified_exception_handler` directly. Test: ValidationError→400, NotAuthenticated→401, PermissionDenied→403, NotFound→404, unexpected Exception→500 (no leak). No DB. ~60 lines. |
| 2.7 | `backend/tests/unit/test_model_methods.py` | Create | [x] Use existing `create_*` fixtures with `@pytest.mark.django_db`. Test: `Appointment.save()` (end_time recalc on start_time change, no recalc without change), `Patient.delete()` (soft delete sets deleted_at, excluded from queries), `Patient.hard_delete()` (permanent removal), `InventoryItem.deduct_stock()` (sufficient, insufficient→InsufficientStockError, exact zero). ~150 lines. |
| 2.8 | `backend/tests/unit/test_auth_service.py`, `test_stock_service.py`, `test_slot_service.py` | Modify | [x] Reclassify DB-dependent tests: remove `@pytest.mark.unit` from pure unit tests (CFDI builder, encryption, Finkok service). Move `test_auth_service.py`, `test_stock_service.py`, `test_slot_service.py` from `unit/` to `integration/` (they use DB). Update file imports accordingly. |

## Phase 3: Cleanup + Verification (2 tasks)

| # | File | Action | Details |
|---|------|--------|---------|
| 3.1 | Full test suite | Verify | [x] Verified: 18 test files exist, all pass ast.parse, pytest.ini correct, test.py syntax OK, markers verified (module-level pytestmark in 3 files, decorator @pytest.mark.unit in 2, @pytest.mark.django_db in 1), run_tests.sh shebang valid, ci.yml valid YAML. Imports: stdlib + pytest resolved; django/rest_framework not installed locally (expected — runs in Docker/CI). |
| 3.2 | `DEPLOYMENT.md` | Modify | [x] Added "## Testing" section with: Docker commands (pytest, -m unit, -m integration, -m contract, -m e2e), coverage (--cov --cov-report=html, htmlcov/index.html), local execution (cd backend && pytest, ./run_tests.sh), CI (GitHub Actions on push/PR to main), marker taxonomy table (unit/integration/contract/e2e/slow). |
